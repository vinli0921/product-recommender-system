...
async def seed_users():
    ...
    async for db in get_db():
        ...
        print(f"📊 Feast users: {feast_filtered} to import, {feast_skipped} skipped (already exist)")
        
        if feast_skipped > 0:
            skipped_ids = users[users["user_id"].astype(str).isin(updated_existing_user_ids)]["user_id"].tolist()
            print(f"   Skipped Feast users: {skipped_ids[:5]}{'...' if len(skipped_ids) > 5 else ''}")
...
def _load_test_user_config():
    ...
    try:
        ...
        return {
            "test_users": [
                {"email": "demo1@example.com", "password": "demo123", "age": 28, "gender": "Female", "preferences": "Electronics,Books", "description": "Fallback test user"}
            ],
            "config": {
...
async def _create_test_users(db, existing_user_ids: set):
    ...
    try:
        ...
        print("🔄 Falling back to configured placeholder user IDs (will have no interaction history)")
    ...
    if len(selected_feast_ids) < len(test_user_templates):
        print(f"⚠️  Only {len(selected_feast_ids)} Feast IDs available, but {len(test_user_templates)} test users configured")
    ...
    print(f"✅ Test users: {created_count} created, {skipped_count} already existed (loaded from config/test_users.yaml)")
    ...
    for user_data in test_users_data:
        ...
        print(f"   [{status}] {user_data['email']} | Password: {user_data['password']} | Feast ID: {user_data['user_id']} | {desc}")
